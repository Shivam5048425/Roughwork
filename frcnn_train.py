...
class AddGaussianNoise(A.ImageOnlyTransform):
    """Custom Albumentations transform to add strong Gaussian noise on the fly."""
    def __init__(self, mean=0.0, std=0.9, always_apply=False, p=1.0):
    ...
    def apply(self, image, **params):
        ...
        # return same dtype (0–255 for visualization, but keep as float32 here)
        return (noisy * 255).astype(np.uint8)

# ---------- Visualization (Original + Labeled Augmentations) ----------
def visualize_multiple_augmentations(dataset, num_samples=60):
    """
...
if __name__ == "__main__":
    ...
    loss_df.to_csv("frcnn_losses.csv", index=False)
    print("✅ Training and validation losses saved to frcnn_losses.csv")
    ...
    metrics.to_csv("frcnn_metrics.csv", index=False)
    print("✅ Final metrics saved to frcnn_metrics.csv")
    ...
    axs[1].set_title("Predictions(Red) Low")
    axs[1].axis("off")

    plt.tight_layout()
    plt.savefig("Predictions vs Ground")
    plt.show()
